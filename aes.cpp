/**
 *  aes.cpp: Ricardo Quezada
 *  A C++ implemenetation of the Rijndael cipher
**/

#include<bits/stdc++.h>
#include"aes.h"
using namespace std;

/**
 *  Precalculated values and constants.
 *  Favor time of execution against memory used
**/

//Constant used in the key schedule (x^i)
unsigned char rcon[14] = 
{
	0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 
	0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d
};

//S_box used in the sub_bytes operation
unsigned char s_box[16][16] = 
{
    {
    	0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5, 
    	0x30, 0x01, 0x67, 0x2B, 0xFE, 0xD7, 0xAB, 0x76
    },
    {
    	0xCA, 0x82, 0xC9, 0x7D, 0xFA, 0x59, 0x47, 0xF0, 
    	0xAD, 0xD4, 0xA2, 0xAF, 0x9C, 0xA4, 0x72, 0xC0
    },
    {
    	0xB7, 0xFD, 0x93, 0x26, 0x36, 0x3F, 0xF7, 0xCC, 
    	0x34, 0xA5, 0xE5, 0xF1, 0x71, 0xD8, 0x31, 0x15
    },
    {
    	0x04, 0xC7, 0x23, 0xC3, 0x18, 0x96, 0x05, 0x9A, 
    	0x07, 0x12, 0x80, 0xE2, 0xEB, 0x27, 0xB2, 0x75
    },
    {
    	0x09, 0x83, 0x2C, 0x1A, 0x1B, 0x6E, 0x5A, 0xA0, 
    	0x52, 0x3B, 0xD6, 0xB3, 0x29, 0xE3, 0x2F, 0x84
    },
    {
    	0x53, 0xD1, 0x00, 0xED, 0x20, 0xFC, 0xB1, 0x5B, 
    	0x6A, 0xCB, 0xBE, 0x39, 0x4A, 0x4C, 0x58, 0xCF
    },
    {
    	0xD0, 0xEF, 0xAA, 0xFB, 0x43, 0x4D, 0x33, 0x85, 
    	0x45, 0xF9, 0x02, 0x7F, 0x50, 0x3C, 0x9F, 0xA8
    },
    {
    	0x51, 0xA3, 0x40, 0x8F, 0x92, 0x9D, 0x38, 0xF5, 
    	0xBC, 0xB6, 0xDA, 0x21, 0x10, 0xFF, 0xF3, 0xD2
    },
    {
    	0xCD, 0x0C, 0x13, 0xEC, 0x5F, 0x97, 0x44, 0x17, 
    	0xC4, 0xA7, 0x7E, 0x3D, 0x64, 0x5D, 0x19, 0x73
    },
    {
    	0x60, 0x81, 0x4F, 0xDC, 0x22, 0x2A, 0x90, 0x88, 
    	0x46, 0xEE, 0xB8, 0x14, 0xDE, 0x5E, 0x0B, 0xDB
    },
    {
    	0xE0, 0x32, 0x3A, 0x0A, 0x49, 0x06, 0x24, 0x5C, 
    	0xC2, 0xD3, 0xAC, 0x62, 0x91, 0x95, 0xE4, 0x79
    },
    {
    	0xE7, 0xC8, 0x37, 0x6D, 0x8D, 0xD5, 0x4E, 0xA9, 
    	0x6C, 0x56, 0xF4, 0xEA, 0x65, 0x7A, 0xAE, 0x08
    },
    {
    	0xBA, 0x78, 0x25, 0x2E, 0x1C, 0xA6, 0xB4, 0xC6, 
    	0xE8, 0xDD, 0x74, 0x1F, 0x4B, 0xBD, 0x8B, 0x8A
    },
    {
    	0x70, 0x3E, 0xB5, 0x66, 0x48, 0x03, 0xF6, 0x0E, 
    	0x61, 0x35, 0x57, 0xB9, 0x86, 0xC1, 0x1D, 0x9E
    },
    {
    	0xE1, 0xF8, 0x98, 0x11, 0x69, 0xD9, 0x8E, 0x94, 
    	0x9B, 0x1E, 0x87, 0xE9, 0xCE, 0x55, 0x28, 0xDF
    },
    {
    	0x8C, 0xA1, 0x89, 0x0D, 0xBF, 0xE6, 0x42, 0x68, 
    	0x41, 0x99, 0x2D, 0x0F, 0xB0, 0x54, 0xBB, 0x16
    }
};

//Inverse used for decrypt
unsigned char inv_s_box[16][16] = 
{
    {
    	0x52, 0x09, 0x6A, 0xD5, 0x30, 0x36, 0xA5, 0x38, 
     	0xBF, 0x40, 0xA3, 0x9E, 0x81, 0xF3, 0xD7, 0xFB
    },
    {
    	0x7C, 0xE3, 0x39, 0x82, 0x9B, 0x2F, 0xFF, 0x87, 
     	0x34, 0x8E, 0x43, 0x44, 0xC4, 0xDE, 0xE9, 0xCB
    },
    {
    	0x54, 0x7B, 0x94, 0x32, 0xA6, 0xC2, 0x23, 0x3D, 
     	0xEE, 0x4C, 0x95, 0x0B, 0x42, 0xFA, 0xC3, 0x4E
    },
    {
    	0x08, 0x2E, 0xA1, 0x66, 0x28, 0xD9, 0x24, 0xB2, 
     	0x76, 0x5B, 0xA2, 0x49, 0x6D, 0x8B, 0xD1, 0x25
    },
    {
    	0x72, 0xF8, 0xF6, 0x64, 0x86, 0x68, 0x98, 0x16, 
     	0xD4, 0xA4, 0x5C, 0xCC, 0x5D, 0x65, 0xB6, 0x92
    },
    {
    	0x6C, 0x70, 0x48, 0x50, 0xFD, 0xED, 0xB9, 0xDA, 
     	0x5E, 0x15, 0x46, 0x57, 0xA7, 0x8D, 0x9D, 0x84
    },
    {
    	0x90, 0xD8, 0xAB, 0x00, 0x8C, 0xBC, 0xD3, 0x0A, 
     	0xF7, 0xE4, 0x58, 0x05, 0xB8, 0xB3, 0x45, 0x06
    },
    {
    	0xD0, 0x2C, 0x1E, 0x8F, 0xCA, 0x3F, 0x0F, 0x02, 
     	0xC1, 0xAF, 0xBD, 0x03, 0x01, 0x13, 0x8A, 0x6B
    },
    {
    	0x3A, 0x91, 0x11, 0x41, 0x4F, 0x67, 0xDC, 0xEA, 
     	0x97, 0xF2, 0xCF, 0xCE, 0xF0, 0xB4, 0xE6, 0x73
    },
    {
    	0x96, 0xAC, 0x74, 0x22, 0xE7, 0xAD, 0x35, 0x85, 
     	0xE2, 0xF9, 0x37, 0xE8, 0x1C, 0x75, 0xDF, 0x6E
    },
    {
    	0x47, 0xF1, 0x1A, 0x71, 0x1D, 0x29, 0xC5, 0x89, 
     	0x6F, 0xB7, 0x62, 0x0E, 0xAA, 0x18, 0xBE, 0x1B
    },
    {
    	0xFC, 0x56, 0x3E, 0x4B, 0xC6, 0xD2, 0x79, 0x20, 
     	0x9A, 0xDB, 0xC0, 0xFE, 0x78, 0xCD, 0x5A, 0xF4
    },
    {
    	0x1F, 0xDD, 0xA8, 0x33, 0x88, 0x07, 0xC7, 0x31, 
     	0xB1, 0x12, 0x10, 0x59, 0x27, 0x80, 0xEC, 0x5F
    },
    {
    	0x60, 0x51, 0x7F, 0xA9, 0x19, 0xB5, 0x4A, 0x0D, 
     	0x2D, 0xE5, 0x7A, 0x9F, 0x93, 0xC9, 0x9C, 0xEF
    },
    {
    	0xA0, 0xE0, 0x3B, 0x4D, 0xAE, 0x2A, 0xF5, 0xB0, 
     	0xC8, 0xEB, 0xBB, 0x3C, 0x83, 0x53, 0x99, 0x61
    },
    {
    	0x17, 0x2B, 0x04, 0x7E, 0xBA, 0x77, 0xD6, 0x26, 
     	0xE1, 0x69, 0x14, 0x63, 0x55, 0x21, 0x0C, 0x7D
    }
};

/**
 *  Precalculated multiply tables under GF(2^8) and
 *  the irreducible polynomial: 1001 1011
 * 
 *  For speed up the algebra in the mix columns steep.
 *	Instead of make the multiply algorithm, only consult the tables.
**/

unsigned char gf_2[256] = 
{
	
};

unsigned char gf_3[256] = 
{
	
};

unsigned char gf_9[256] =
{
	
};

unsigned char gf_11[256] = 
{
	
};

unsigned char gf_13[256] =
{
	
};

unsigned char gf_14[256] = 
{
	
};

/**
 *  key_schedule: return the expansion of the given key.
 *  The type 'Data' is a vector of Array (see aes.h).
**/

Data key_schedule(Array key)
{
	int size = key.size();
	int rounds = (size == 32) ? 14 :
				 (size == 24) ? 12 :
				 10;
	Data keys;
	keys.push_back(key);
	Array tmp = schedule_core(key, 0);

	for(int i=0; i<rounds; i++){
		for(int j=0; j<size; j++){
			tmp[j % 4] ^= key[j];
			key[j] = tmp[j % 4];
		}
		keys.push_back(key);
		if(i == rounds - 1) break;
		tmp = schedule_core(keys[i], i+1);
	}

	return keys;
}

/**
 *  schedule_core: Main private function used by the key schedule.
 *  Return the subkey which is used for xoring and generate
 *  the next expansions in the loop of the schedule.
 *
 *  The function make three operations: the rotation, the s_box
 *  substitution, and the xor with the rcon value (x^i) in each iteration.
**/

Array schedule_core(Array &key, int iterator)
{
	int size = key.size();
	Array subkey;
	subkey.push_back(key[size - 4]);
	subkey.push_back(key[size - 3]);
	subkey.push_back(key[size - 2]);
	subkey.push_back(key[size - 1]);

	rotate(subkey);
	sub_bytes(subkey, ENCRYPTION_MODE);
	subkey[0] ^= rcon[iterator];
	return subkey;
}

/**
 *  rotate: Make a simply circular rotation with a four bytes subkey.
 *  The opertion is made over the same argument (passed by reference),
 *  in consequence, return void.
**/

void rotate(Array &subkey)
{
	unsigned char aux = subkey[0];
	subkey[0] = subkey[1];
	subkey[1] = subkey[2];
	subkey[2] = subkey[3];
	subkey[3] = aux;
}

/**
 *  operate: main public function for encrypt or decrypt.
 *  Receive the data, the key, and the mode.
 *  The encryption/decryption is made over the same Array.
 *  The numbers of rounds depends of the length of te given key.
**/

void operate(Array &data, Data &key_expansion, int mode)
{
	int rounds = key_expansion.size();

	//First round
	add_round_key(data, 
		(mode == ENCRYPTION_MODE) ? key_expansion[0] : 
									key_expansion[rounds - 1]);

	//Center rounds
	for(int i = 1; i < rounds - 1; i++){
		if(mode == ENCRYPTION_MODE){
			sub_bytes(data, mode);
			shift_rows(data, mode);
			mix_columns(data, mode);
			add_round_key(data, key_expansion[i]);
		}else{
			shift_rows(data, mode);
			sub_bytes(data, mode);
			add_round_key(data, key_expansion[rounds - i - 1]);
			mix_columns(data, mode);
		}
	}

	//Final round
	if(mode == ENCRYPTION_MODE){
		sub_bytes(data, mode);
		shift_rows(data, mode);
		add_round_key(data, key_expansion[rounds - 1]);
	}else{
		shift_rows(data, mode);
		sub_bytes(data, mode);
		add_round_key(data, key_expansion[0]);
	}
}

/**
 *  add_round_key: Simply xor betwen the data and the given sub key.
**/

void add_round_key(Array &data, Array &key)
{
	for(int i=0; i<data.size(); i++)
		data[i] ^= key[i];
}

/**
 *  sub_bytes: Susbstitution of the data values with the s_box values.
 *  The mode is for known wich table use.
**/

void sub_bytes(Array &data, int mode)
{
	for(int i=0; i<data.size(); i++)
		data[i] = (mode == ENCRYPTION_MODE) ? 
				  s_box[data[i] / 16][data[i] % 16] :
				  inv_s_box[data[i] / 16][data[i] % 16];
}

/**
 *  shift_rows: Shift operation of the algorithm.
 *  The number of shift per rows depends of the key size:
 *
 *  For 128 and 192 bits: 1, 2, 3
 *  For 256: 1, 3, 4
 *  This is done over the last three rows (the first dont change).
 *  The function only acts like a buffer.
 *  The shift itself is done by the next function.
**/

void shift_rows(Array &data, int mode)
{
	int shifts[3];
	int columns = data.size() / 4;
	if(data.size() == 32){
		//256 bits
		if(mode == ENCRYPTION_MODE)
			{shifts[0] = 1; shifts[1] = 3; shifts[2] = 4;}
		else{
			shifts[0] = columns - 1; 
			shifts[1] = columns - 3; 
			shifts[2] = columns - 4;
		}
	}else{
		//128 or 192 bits
		if(mode == ENCRYPTION_MODE)
			{shifts[0] = 1; shifts[1] = 2; shifts[2] = 3;}
		else{
			shifts[0] = columns - 1; 
			shifts[1] = columns - 2; 
			shifts[2] = columns - 3;
		}
	}

	for(int i=1; i<4; i++)
		shift_row(data, i, shifts[i - 1]);
}

/**
 *  shift_row: shift the given row in the given data 'shift' places at right.
**/

void shift_row(Array &data, int row, int shift)
{
	int size = data.size() / 4; //number of columns
	Array aux;
	for(int i=0; i<size; i++) 
		aux.push_back(data[row + 4*i]);
	for(int i=0; i<size; i++)
		data[row + 4*i] =  aux[(i + shift) % size];
}

/**
 *  mix_columns: Multiply each column by a fixed polynomial in GF(2^8)
 *  For encrypt: 3x^3 + x^2 + x + 2,
 *  And for decrypt (the inverse): 11x^3 + 13x^2 + 9x + 14
 *  Instead of the multiplications are done conults to the precalculateg
 *  tables gf_N
**/

void mix_columns(Array &data, int mode)
{
	unsigned char aux[4];
	for(int i = 0; i < data.size() / 4; i++){

		if(mode == ENCRYPTION_MODE){
			aux[0] = gf_2[data[4 * i]] ^ gf_3[data[4 * i + 1]] ^ 
					 data[4 * i + 2] ^ data[4 * i + 3];
			aux[1] = data[4 * i] ^ gf_2[data[4 * i + 1]] ^ 
					 gf_3[data[4 * i + 2]] ^ data[4 * i + 3];
			aux[2] = data[4 * i] ^ data[4 * i + 1] ^ 
					 gf_2[data[4 * i + 2]] ^ gf_3[data[4 * i + 3]];
			aux[3] = gf_3[data[4 * i]] ^ data[4 * i + 1] ^ 
					 data[4 * i + 2] ^ gf_2[data[4 * i + 3]];
		}else{
			aux[0] = gf_14[data[4 * i]] ^ gf_11[data[4 * i + 1]] ^ 
					 gf_13[data[4 * i + 2]] ^ gf_9[data[4 * i + 3]];
			aux[1] = gf_9[data[4 * i]] ^ gf_14[data[4 * i + 1]] ^ 
					 gf_11[data[4 * i + 2]] ^ gf_13[data[4 * i + 3]];
			aux[2] = gf_13[data[4 * i]] ^ gf_9[data[4 * i + 1]] ^ 
					 gf_14[data[4 * i + 2]] ^ gf_11[data[4 * i + 3]];
			aux[3] = gf_11[data[4 * i]] ^ gf_13[data[4 * i + 1]] ^ 
					 gf_9[data[4 * i + 2]] ^ gf_14[data[4 * i + 3]];
		}

		data[4 * i] = aux[0];
		data[4 * i + 1] = aux[1];
		data[4 * i + 2] = aux[2];
		data[4 * i + 3] = aux[3];
	}
}

